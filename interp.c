...
void tl_interp_init(tl_interp *in) {
	...
	top_frm = _tl_frm_set("tl-call-with-current-continuation", tl_new_cfunc(in, tl_cf_call_with_current_continuation), top_frm);

	/*
	top_frm = _tl_frm_set("tl-apply", tl_new_cfunc(in, tl_cf_apply), top_frm);
	*/

	top_frm = _tl_frm_set("tl-read", tl_new_cfunc(in, tl_cf_read), top_frm);

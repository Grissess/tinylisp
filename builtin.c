...
void _tl_cf_error_k(tl_interp *in, tl_object *result, tl_object *_) {
	tl_error_set(in, tl_first(result));
	tl_cfunc_return(in, in->true_);
}

void tl_cf_error(tl_interp *in, tl_object *args, tl_object *_) {
	if(args) {
		tl_eval_and_then(in, tl_first(args), NULL, _tl_cf_error_k);
	} else {
...
void _tl_cf_display_k(tl_interp *in, tl_object *result, tl_object *next) {
	tl_print(in, tl_first(result));
	if(next) {
		in->printf(in->udata, "\t");
		tl_eval_and_then(in, tl_first(next), tl_next(next), _tl_cf_display_k);
	} else {
		in->printf(in->udata, "\n");
		tl_cfunc_return(in, in->true_);
	}
...
void tl_cf_display(tl_interp *in, tl_object *args, tl_object *_) {
...
void _tl_cf_evalin_k(tl_interp *in, tl_object *args, tl_object *_) {
...
void tl_cf_evalin(tl_interp *in, tl_object *args, tl_object *_) {
	arity_n(in, args, 3, "eval-in&");
	tl_eval_all_args(in, args, NULL, _tl_cf_evalin_k);
}

void _tl_cf_call_with_current_continuation_k(tl_interp *in, tl_object *args, tl_object *_) {
	tl_object *cont = tl_new_cont(in, in->env, in->conts, in->values);
	tl_push_apply(in, 1, tl_first(args), in->env);
	tl_values_push(in, cont);
}

void tl_cf_call_with_current_continuation(tl_interp *in, tl_object *args, tl_object *_) {
	arity_1(in, args, "call-with-current-continuation");
	tl_eval_all_args(in, args, NULL, _tl_cf_call_with_current_continuation_k);
}

void _tl_cf_cons_k(tl_interp *in, tl_object *args, tl_object *_) {
	tl_cfunc_return(in, tl_new_pair(in, tl_first(args), tl_first(tl_next(args))));
}

void tl_cf_cons(tl_interp *in, tl_object *args, tl_object *_) {
	arity_1(in, args, "cons");
	tl_eval_all_args(in, args, NULL, _tl_cf_cons_k);
}

void _tl_cf_car_k(tl_interp *in, tl_object *args, tl_object *_) {
	tl_cfunc_return(in, tl_first(tl_first(args)));
}

void tl_cf_car(tl_interp *in, tl_object *args, tl_object *_) {
	arity_1(in, args, "car");
	tl_eval_all_args(in, args, NULL, _tl_cf_car_k);
}

void _tl_cf_cdr_k(tl_interp *in, tl_object *args, tl_object *_) {
	tl_cfunc_return(in, tl_next(tl_first(args)));
}

void tl_cf_cdr(tl_interp *in, tl_object *args, tl_object *_) {
	arity_1(in, args, "cdr");
	tl_eval_all_args(in, args, NULL, _tl_cf_cdr_k);
}

void _tl_cf_null_k(tl_interp *in, tl_object *args, tl_object *_) {
	tl_cfunc_return(in,  _boolify(!tl_first(args)));
}

void tl_cf_null(tl_interp *in, tl_object *args, tl_object *_) {
...
void _tl_cf_env_k(tl_interp *in, tl_object *result, tl_object *_) {
...
void tl_cf_env(tl_interp *in, tl_object *args, tl_object *_) {
...
void _tl_cf_setenv_global_k(tl_interp *in, tl_object *result, tl_object *_) {
	in->env = tl_first(result);
	tl_cfunc_return(in, in->true_);
}

void _tl_cf_setenv_k(tl_interp *in, tl_object *args, tl_object *_) {
...
void tl_cf_setenv(tl_interp *in, tl_object *args, tl_object *_) {
	tl_object *first = tl_first(args), *next = tl_first(tl_next(args));
...
void tl_cf_topenv(tl_interp *in, tl_object *args, tl_object *_) {
	tl_cfunc_return(in, in->top_env);
}

void _tl_cf_concat_k(tl_interp *in, tl_object *args, tl_object *_) {
...
void tl_cf_concat(tl_interp *in, tl_object *args, tl_object *_) {
	tl_eval_all_args(in, args, NULL, _tl_cf_concat_k);
}

void _tl_cf_length_k(tl_interp *in, tl_object *result, tl_object *_) {
	verify_type(in, tl_first(result), sym, "length");
	tl_cfunc_return(in, tl_new_int(in, strlen(tl_first(result)->str)));
}

void tl_cf_length(tl_interp *in, tl_object *args, tl_object *_) {
	tl_eval_and_then(in, tl_first(args), NULL, _tl_cf_length_k);
}

void _tl_cf_add_k(tl_interp *in, tl_object *args, tl_object *_) {
	long res = 0;
	for(tl_list_iter(args, val)) {
...
void tl_cf_add(tl_interp *in, tl_object *args, tl_object *_) {
	tl_eval_all_args(in, args, NULL, _tl_cf_add_k);
}

void _tl_cf_sub_k(tl_interp *in, tl_object *args, tl_object *_) {
	long phase = 0;
	long res = 0;
	for(tl_list_iter(args, val)) {
		verify_type(in, val, int, "-");
...
void tl_cf_sub(tl_interp *in, tl_object *args, tl_object *_) {
	tl_eval_all_args(in, args, NULL, _tl_cf_sub_k);
}

void _tl_cf_mul_k(tl_interp *in, tl_object *args, tl_object *_) {
	long res = 1;
	for(tl_list_iter(args, val)) {
		verify_type(in, val, int, "*");
		res *= val->ival;
...
void tl_cf_mul(tl_interp *in, tl_object *args, tl_object *_) {
	tl_eval_all_args(in, args, NULL, _tl_cf_mul_k);
}

void _tl_cf_div_k(tl_interp *in, tl_object *args, tl_object *_) {
	long phase = 0;
	long res = 1;
	for(tl_list_iter(args, val)) {
		verify_type(in, val, int, "/");
...
void tl_cf_div(tl_interp *in, tl_object *args, tl_object *_) {
	tl_eval_all_args(in, args, NULL, _tl_cf_div_k);
}

void _tl_cf_mod_k(tl_interp *in, tl_object *args, tl_object *_) {
	long phase = 0;
	long res = 1;
	for(tl_list_iter(args, val)) {
		verify_type(in, val, int, "%");
...
void tl_cf_mod(tl_interp *in, tl_object *args, tl_object *_) {
	tl_eval_all_args(in, args, NULL, _tl_cf_mod_k);
}

void _tl_cf_eq_k(tl_interp *in, tl_object *args, tl_object *_) {
...
void tl_cf_eq(tl_interp *in, tl_object *args, tl_object *_) {
	tl_eval_all_args(in, args, NULL, _tl_cf_eq_k);
}

void _tl_cf_less_k(tl_interp *in, tl_object *args, tl_object *_) {
	tl_object *a = tl_first(args), *b = tl_first(tl_next(args));
	if(tl_is_int(a) && tl_is_int(b)) {
		tl_cfunc_return(in, _boolify(a->ival < b->ival));
	}
...
void tl_cf_less(tl_interp *in, tl_object *args, tl_object *_) {
	tl_eval_all_args(in, args, NULL, _tl_cf_less_k);
}

void _tl_cf_nand_k(tl_interp *in, tl_object *args, tl_object *_) {
	int a = _unboolify(in, tl_first(args)), b = _unboolify(in, tl_first(tl_next(args)));
	tl_cfunc_return(in, _boolify(!(a && b)));
}

void tl_cf_nand(tl_interp *in, tl_object *args, tl_object *_) {
	tl_eval_all_args(in, args, NULL, _tl_cf_nand_k);
}

void _tl_cf_type_k(tl_interp *in, tl_object *result, tl_object *_) {
...
void tl_cf_type(tl_interp *in, tl_object *args, tl_object *_) {
...
void tl_cf_gc(tl_interp *in, tl_object *args, tl_object *_) {
	tl_gc(in);
	tl_cfunc_return(in, in->true_);
}

void tl_cf_read(tl_interp *in, tl_object *args, tl_object *_) {
